...
const unsigned int get_hash(Card *cards, std::vector<unsigned int> &indexes) {
  ...
  return cards[indexes[0]].prime * cards[indexes[1]].prime *
         cards[indexes[2]].prime * cards[indexes[3]].prime *
         cards[indexes[4]].prime;
}

unsigned int check_flush(Card *cards, const unsigned int hash,
                         std::vector<unsigned int> &indexes) {
  if (cards[indexes[0]].suit & cards[indexes[1]].suit & cards[indexes[2]].suit &
      cards[indexes[3]].suit & cards[indexes[4]].suit) {
    return flush_dic[hash];
  }
  return 0;
}

void set_best(std::pair<Rank, unsigned int> &cur,
              std::pair<Rank, unsigned int> &best) {
...
int get_p1_result(std::pair<Rank, unsigned int> &p1,
                  std::pair<Rank, unsigned int> &p2) {
